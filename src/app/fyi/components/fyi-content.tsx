        ...
        <div className="max-w-4xl mx-auto">
                ...
                >
                  ...
                  {doc.elements && (
                    <div className="space-y-4 mb-10">
                      <p className="text-[10px] uppercase font-black text-primary/60 tracking-[0.3em] mb-4">
                      ...
                      <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-3">
                        {doc.elements.map((el, idx) => (
                          <li
                            key={idx}
                            className="flex items-center gap-3 text-sm font-bold text-muted-foreground border-b border-border/30 pb-2"
                          >
                    ...
                    <div className="space-y-6 mb-10">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {doc.scenarios.map((scen, idx) => (
                          <div
                            key={idx}
                            className="bg-muted/30 p-5 border-l-2 border-primary/30 hover:bg-muted/50 transition-colors"
                          >
                  ...
                  {doc.example && (
                    <div className="mt-8 bg-primary/5 p-8 border border-primary/10 italic text-base relative">
                      ...
                      <p className="font-bold text-primary mb-3 text-[10px] uppercase tracking-[0.4em]">
                        Exemplo Robusto:
                      </p>
              ...
              <table className="w-full min-w-[600px]">
                <thead>
                  <tr className="border-b border-primary/20">
                    <th className="text-left py-6 px-4 uppercase text-xs font-black text-primary tracking-[0.2em]">
                      Documento
                    </th>
                    <th className="text-center py-6 px-4 uppercase text-xs font-black text-primary tracking-[0.2em]">
                      Gera IVA
                    </th>
                    <th className="text-center py-6 px-4 uppercase text-xs font-black text-primary tracking-[0.2em]">
                      Liquidação
                ...
                <tbody>
                  {comparisonTable.map((row, i) => (
                    <tr
                      key={i}
                      className="border-b border-border/50 hover:bg-primary/2 transition-colors group"
                    >
                      <td className="py-7 px-4 font-black text-foreground uppercase text-lg group-hover:text-primary transition-colors">
                        {row.doc}
                      </td>
                      <td className="py-7 px-4 text-center text-base font-bold text-muted-foreground">
                        {row.iva}
                      </td>
                      <td className="py-7 px-4 text-center text-base font-bold text-muted-foreground">
                        {row.payment}
                        ...
                        <span
                          className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-test-test-none ${
                            row.fiscal === "Sim"
                              ...
                              : row.fiscal === "Não"
                                ? "bg-red-500/10 text-red-600 dark:text-red-500 border border-red-500/30"
          ...
          <div className="text-center space-y-10 py-16">
            ...
            <p className="text-sm text-muted-foreground font-medium">
              MindGest ERP: O melhor software de facturação certificado pela AGT
              em Angola.
            </p>
          </div>
        </div>
